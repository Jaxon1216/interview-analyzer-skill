...
# This file is a template. During skill generation, interview-analyzer-skill is replaced
...
# Constants
# ---------------------------------------------------------------------------
SKILL_NAME="interview-analyzer-skill"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
SKILL_MD="${SCRIPT_DIR}/SKILL.md"
VERSION="$(awk '
    NR==1 && $0=="---" { in_frontmatter=1; next }
    in_frontmatter && $0=="---" { exit }
    in_frontmatter && $1=="version:" { print $2; exit }
...
extract_skill_description() {
    ...
    while IFS= read -r line; do
        ...
        if $in_description_block; then
            case "$line" in
                [[:space:]]*)
                    part="$(printf '%s' "$line" | sed 's/^[[:space:]]*//')"
                    [ -n "$part" ] && desc="${desc}${desc:+ }${part}"
                    ;;
                *)
                    break
                    ;;
        ...
        case "$line" in
            description:*)
                value="$(printf '%s' "$line" | sed 's/^[[:space:]]*description:[[:space:]]*//')"
                case "$value" in
                    ""|">-"|">")
