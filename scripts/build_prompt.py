...
def build_prompt(
    ...
    lines.extend(
        [
            "## 输出要求（摘要）",
            "",
            "- 面经：先抽取 4～6 个架构支柱，再写简介与 bullet；每条 bullet 先交代问题/演进和职责，再写机制、约束/边界、结果，最后填「简历 → 面试展开」表并按表出题。",
            "- 面经 bullet：对照领域中立 few-shot 的正例、反例和改写对照，只学习表达结构；禁止复制样例的项目名、数字、领域名词或指标。",
            "",
            "## Bullet few-shot（仅学习结构，不复制素材）",
            "",
            "**合格**：针对异常处理逻辑散落、改动容易波及主流程的问题，将错误处理收敛为分层容错机制：按错误类型区分有限重试、降级与快速失败，并统一输出可观测结果，降低新增场景对主链路的改动面；线上收益需通过基线对比验证。",
            "",
            "**不合格**：接入缓存和重试，优化接口请求。",
            "",
